...
<script>
    ...
    function refresh() {
        location.href = '<?=base_url("heijunka_wos_print")?>';
    }
            ...
            success:function(r) {
                ...
                if(action != "Refresh"){
                    if(d.status == "sukses"){
                        ...
                        if(next == "Selesai"){
                            swal.fire("Sukses","Proses Heijunka telah selesai","success");
                        }else{
	...
	function reload() {
		window.location.href = '<?= base_url("heijunka_wos_print") ?>';
	}

    $("#upload-wos").change(function(e) {
		console.log(e);
		$("#customFile");
        loading("Sedang Upload...","Mohon tunggu sebentar");
        $("#import_wos_backup_print").submit();
    });
</script>
<script>
