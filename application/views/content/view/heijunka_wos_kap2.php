...
<head>
    ...
    if ($this->input->get('Increment') == 'Yes') {
        $row_master = $this->model->gds_heijunka("master_kap2", "*", "No != '' ORDER BY Heijunka_Model ASC, Heijunka_Suffix ASC, Heijunka_Color,heijunka_tone ASC", "result");
        foreach ($row_master as $row_master) {
            $data = [
                "No" => $i++,
            ];
            $update_no = $this->model->update_heijunka("master_kap2", "SAPNIK = '" . $row_master->SAPNIK . "'", $data);
        }
        $row_compo = $this->model->gds_heijunka("master_kap2", "*", "No != '' ORDER BY Heijunka_Model ASC, Heijunka_Suffix ASC, Heijunka_Color ASC", "result");
        foreach ($row_compo as $row_compo) {
            $Before = $row_compo->No - 1;
            $After = $row_compo->No + 1;
            $B = $this->model->gds_heijunka("master_kap2", "*", "No = '$Before'", "row");
            $A = $this->model->gds_heijunka("master_kap2", "*", "No = '$After'", "row");
            ...
            $update_compo = $this->model->update_heijunka("master_kap2", "SAPNIK = '" . $row_compo->SAPNIK . "'", $dupdate_compo);
            if ($row_compo->Bot_Color != $B->Bot_Color) {
                $dupdate_no_bot = [
                    "No_Bot_Color" => "1",
                ];
                $update_no_bot = $this->model->update_heijunka("master_kap2", "SAPNIK = '" . $row_compo->SAPNIK . "'", $dupdate_no_bot);
            } else {
    ...
    if (!empty($this->input->get('CEK_BOTH'))) {
        ...
        $clear = $this->model->update_heijunka("master_kap2", "Status_Bot_Color = 'X'", $dupdate_both);
        $row_mark_status = $this->model->gds_heijunka("master_kap2", "SAPNIK", "No_Bot_Color = '3'", "result");
        if (!empty($row_mark_status)) {
            foreach ($row_mark_status as $row_mark_status) {
                $dupdate = ['Status_Bot_Color' => 'X'];
                $update = $this->model->update_heijunka("master_kap2", "SAPNIK = '" . $row_mark_status->SAPNIK . "'", $dupdate);
            }
            if (!$update) {
            ?>
...
<?php
    ...
    $history_color = $this->model->gds_heijunka("history_kap2", "*", "Heijunka = 'Color'", "row");
    $history_sfx = $this->model->gds_heijunka("history_kap2", "*", "Heijunka = 'Suffix'", "row");
    $history_model = $this->model->gds_heijunka("history_kap2", "*", "Heijunka = 'Model'", "row");
    $history_transmisi = $this->model->gds_heijunka("history_kap2", "*", "Heijunka = 'Transmisi'", "row");
        ...
        <div class="col-12">
            <div class="row d-flex bg-info">
                <div class="col-12" align="center">
                    <h1 class="m-0 text-white mb-2 mt-2">HEIJUNKA WOS KAP2</h1>
                </div>
            ...
            <div class="card">
                        ...
                        <div class="col pr-1 pl-1">
                            <div class="card">
                                ...
                                <div class="card-body p-2 text-center">
                                    ...
                                    ?>
									<a href="<?= base_url("dummy_process_kap2?start_no=".$start_no) ?>" class="btn btn-info" style="font-size:8pt;" onclick="loading('Proses D74A Link...','Mohon tunggu sampai proses selesai.')">PROCESS</a>
                                </div>
								<?php
								//STATUS D74A-LINK
								$status_d74a_link = $this->model->gds_heijunka("history_kap2","Status","Heijunka = 'D74A-LINK'","row");
                        ...
                        <div class="col pl-1">
                            <div class="card">
                                ...
                                <div class="card-body p-1">
                                    <form action="<?=base_url("import_woskap2")?>" class="mb-0" method="post" id="import_wos" enctype="multipart/form-data" style="height:36px;">
                ...
                <!-- <?= $this->session->userdata("nik1")." ".$this->session->userdata("nik2"); ?> -->
                <div class="card-body p-1" style="">
                    <div class="row mt-0">
                        <div class="col-12">
                            <div class="row">
                                <?php
                                $get_unit = $this->model->gds_heijunka("master_kap2","COUNT(SAPNIK) as total,Model","No != '' GROUP BY Model","result");
                                $total_unit = $this->model->gds_heijunka("master_kap2","COUNT(SAPNIK) as total","No != ''","row");
                                $total_td_link = $this->model->gds_heijunka("master_td_link_kap2","COUNT(SAPNIK) as total","No != ''","row");
                                if(!empty($get_unit)){
                        ...
                        <div class="col-12 pr-3">
                            ...
                            <input type="hidden" id="tipe_switch" value="1">
                            <table border="1" class="table_WOS w-100" style="font-size: 6pt;">
                                ...
                                if($this->input->get("group_model") == "yes"){
                                    $data_wos = $this->model->union_heijunka_kap2("ORDER BY batch,FIELD(Model,'D26A','D74A','D52B'), No DESC");
                                }else{
                                ...
                                $count_td_link = $this->model->gds_heijunka("master_td_link_kap2","COUNT(No) as count","No !=","row");
                                foreach ($data_wos as $data_wos) {
									...
									if($ng_model > 4){
										$bg_ng_model = "bg-warning text-danger ng-model";
										$text_ng = "Model NG";
									}else{
										$bg_ng_model = "bg-light text-dark";
										$text_ng = "";
									}
									...
									<tr id="tr_<?= ($No+1); ?>" data-id="" class="<?= $bg_model; ?>" align="center" <?= $choosen; ?>>
										...
										<?=$tone_td?>
										<td class="<?=$color_batch[$data_wos->batch];?>"><?= $data_wos->batch; ?></td>
										<td class="<?=$bg_ng_model?>"><?= $text_ng; ?></td>
									</tr>
									<?php
                                    $No++;
                                    $i++;
        ...
        <div class="modal-dialog modal-sm modal-dialog-centered" role="document">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <form action="<?=base_url("dup_kap2")?>" method="post" id="form-download" <?php if(empty($this->input->get("no_dummy"))){ echo 'target="_blank"'; } ?>>
                        <div class="row">
                            ...
                            <div class="col-12 mb-2">
                                ...
                                <select name="dummy" id="dummy" class="form-control">
                                    <option <?php if(empty($this->input->get("no_dummy"))){ echo "selected"; } ?>>YES</option>
                                    <option <?php if(!empty($this->input->get("no_dummy"))){ echo "selected"; } ?>>NO</option>
                                </select>
                            </div>
                            <div class="col-12 mb-2">
                            ...
                            <?php
                            if(empty($this->input->get("no_dummy"))){
                                ?>
                                <div class="col-12 mb-2">
...
<?php $this->load->view("js/heijunka_wos_kap2"); ?>
