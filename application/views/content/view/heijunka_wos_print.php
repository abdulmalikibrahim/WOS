...
<head>
    ...
    if ($this->input->get('Increment') == 'Yes') {
        $row_master = $this->model->gds_heijunka("master_print", "*", "No != '' ORDER BY Heijunka_Model ASC, Heijunka_Suffix ASC, Heijunka_Color,heijunka_tone ASC", "result");
        foreach ($row_master as $row_master) {
            $data = [
                "No" => $i++,
            ];
            $update_no = $this->model->update_heijunka("master_print", "SAPNIK = '" . $row_master->SAPNIK . "'", $data);
        }
        $row_compo = $this->model->gds_heijunka("master_print", "*", "No != '' ORDER BY Heijunka_Model ASC, Heijunka_Suffix ASC, Heijunka_Color ASC", "result");
        foreach ($row_compo as $row_compo) {
            $Before = $row_compo->No - 1;
            $After = $row_compo->No + 1;
            $B = $this->model->gds_heijunka("master_print", "*", "No = '$Before'", "row");
            $A = $this->model->gds_heijunka("master_print", "*", "No = '$After'", "row");
            ...
            $update_compo = $this->model->update_heijunka("master_print", "SAPNIK = '" . $row_compo->SAPNIK . "'", $dupdate_compo);
            if ($row_compo->Bot_Color != $B->Bot_Color) {
                $dupdate_no_bot = [
                    "No_Bot_Color" => "1",
                ];
                $update_no_bot = $this->model->update_heijunka("master_print", "SAPNIK = '" . $row_compo->SAPNIK . "'", $dupdate_no_bot);
            } else {
    ...
    if (!empty($this->input->get('CEK_BOTH'))) {
        ...
        $clear = $this->model->update_heijunka("master_print", "Status_Bot_Color = 'X'", $dupdate_both);
        $row_mark_status = $this->model->gds_heijunka("master_print", "SAPNIK", "No_Bot_Color = '3'", "result");
        if (!empty($row_mark_status)) {
            foreach ($row_mark_status as $row_mark_status) {
                $dupdate = ['Status_Bot_Color' => 'X'];
                $update = $this->model->update_heijunka("master_print", "SAPNIK = '" . $row_mark_status->SAPNIK . "'", $dupdate);
            }
            if (!$update) {
            ?>
        ...
        <div class="col-12">
            <div class="row d-flex bg-info">
                <div class="col-12" align="center">
                    <h1 class="m-0 text-white mb-2 mt-2">BACKUP VEHICLE CARD PRINT</h1>
                </div>
                    ...
                    <div class="row">
                        <div class="col-lg-4 pl-3">
                            <div class="card border-0">
                                <div class="card-header p-2 text-center bg-info">
                                    <h6 class="m-0 text=-">UPLOAD WOS</h6>
                                </div>
                                <div class="card-body p-0 pt-2">
                                    <form action="<?=base_url("import_wos_backup_print")?>" class="mb-0" method="post" id="import_wos_backup_print" enctype="multipart/form-data" style="height:36px;">
                                        <div class="input-group">
											...
											<button class="btn btn-info ml-1">Upload</button>
                                        </div>
                                    </form>
                                </div>
                            </div>
                ...
                <div class="card-body p-1" style="">
                    <div class="row mt-0">
                        <div class="col-12">
                            <div class="row">
                                <?php
                                $get_unit = $this->model->gds_heijunka("master_print","COUNT(SAPNIK) as total,Model","No != '' GROUP BY Model","result");
                                $total_unit = $this->model->gds_heijunka("master_print","COUNT(SAPNIK) as total","No != ''","row");
                                if(!empty($get_unit)){
                                    foreach ($get_unit as $get_unit) {
										...
										<div class='col font-weight-bold text-center'>
											".$get_unit->Model." : ".$get_unit->total." (".$percent_unit.")
											<div class='mt-2 mb-2'>
												<a class='btn btn-sm btn-success text-white justify-content-bottom' href='".base_url('print_card?model='.$get_unit->Model)."'><i class='fas fa-print pr-2'></i>Print Card</a>
											</div>
										</div>";
                                    }
                                }
...
<?php $this->load->view("js/heijunka_wos_print"); ?>
